...
spkview_list = ['hI', 'BI', 'jI', 'wo', 'waka', 'lagaBaga', 'lagAtAra', 'kewala']
...
def check_verb(word_data):
    ...
    if '-' in word_data[1]:
        ...
        if word_data[4] != '':
            rel = word_data[4].strip().split(':')[1]
            if rel in ('rpk','rbk', 'rvks', 'rbks','rsk'): #nonfinite verb form recognition
                return True
    return False
...
def check_nominal_verb(word_data):
    ...
    if word_data[4].strip() != '':
        ...
        if relation in rel_list and gnp_info == '': #nominal rule
            return True
    return False

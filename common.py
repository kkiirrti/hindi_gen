...
def getVerbGNP_new(verbs_data, seman_data, depend_data, processed_nouns, processed_pronouns):
    '''
    If yA tam of main verb
        if CP and no K2 then:
                verb.GNP= CP.GNP
        else:
        if no CP and K2 exists and K2 is non anim:
                verb.GNP = K2.GNP
        if no CP and K2 exists and K2 is anim:
                verb.GNP = default msa

      If not yA tam, then not from given TAM list
                verb.GNP = k1.gnp
